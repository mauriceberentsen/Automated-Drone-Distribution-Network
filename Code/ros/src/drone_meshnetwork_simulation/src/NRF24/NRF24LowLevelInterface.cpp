...
NRF24LowLevelInterface::NRF24LowLevelInterface(
    NRF24HighLevelInterface *_highLevelInterface )
                                    ...
                                    const uint64_t _broadcastAddress )
{
 ...
 this->antennaThread =
     std::thread( std::bind( &NRF24LowLevelInterface::runAntenna, this ) );
}
...
void NRF24LowLevelInterface::runAntenna( )
{
 while ( currentState != OFF ) {
  if ( currentState == RECEIVING ) { HandleIncomingMessages( ); }
...
void NRF24LowLevelInterface::setNodeIdReadAddress(
    const uint64_t _NodeIdReadAddress )
{
 this->NodeIdReadAddress = _NodeIdReadAddress;
 radio.openReadingPipe( 2, _NodeIdReadAddress );
}
void NRF24LowLevelInterface::setBroadcastAddress(
    const uint64_t _broadcastAddress )
{
