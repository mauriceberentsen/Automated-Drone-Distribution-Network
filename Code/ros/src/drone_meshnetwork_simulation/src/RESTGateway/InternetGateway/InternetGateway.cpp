...
void InternetGateway::init( Communication::Internet::IGatewayCommands* IGC,
                            uint8_t _threads )
{
...
void InternetGateway::CreateDescription( )
{
 ...
 Routes::Post( router, "/DroneMovement/:id/:latitude/:longitude/:height",
...
{
 this->InternetConnectionThread =
     std::thread( std::bind( &InternetGateway::InternetService, this ) );
}
...
void InternetGateway::RequestDroneMovement( const Rest::Request& request,
                                            Http::ResponseWriter response )
{
 ...
 response.send( Http::Code::Ok, std::to_string( id ) + "  " +
