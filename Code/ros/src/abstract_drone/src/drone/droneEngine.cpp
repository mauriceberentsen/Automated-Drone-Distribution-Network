...
namespace gazebo
{
  class DroneEngine : public ModelPlugin
  {
    public: void Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
    {
      ...
      ros::SubscribeOptions so =
        ros::SubscribeOptions::create<abstract_drone::Location>(
            move_to_TopicName,
            1,
            boost::bind(&DroneEngine::OnRosMsg_Pos, this, _1),
            ros::VoidPtr(), &this->rosQueue);
      this->rosSub = this->rosNode->subscribe(so);
      
      this->rosPostmasterPub = this->rosNode->advertise<abstract_drone::nodeInfo>("/postMaster",100);
      // Spin up the queue helper thread.
      this->rosQueueThread =
        std::thread(std::bind(&DroneEngine::QueueThread, this));

      ROS_WARN("Loaded DroneEngine Plugin with parent...%s", this->model->GetName().c_str());
    ...
    bool atGoal()
    {
      ...
      return  (goal.Pos().X() < pose.Pos().X() + presision) && (goal.Pos().X() > pose.Pos().X() - presision) 
      &&      (goal.Pos().Y() < pose.Pos().Y() + presision) && (goal.Pos().Y() > pose.Pos().Y() - presision) 
      &&      (goal.Pos().Z() < pose.Pos().Z() + presision) && (goal.Pos().Z() > pose.Pos().Z() - presision) ;
    }
    ...
    void MoveModelsPlane()
    {
      ...
      ROS_WARN("exec move model");
        ...
        moving = true;
        ignition::math::Vector3d curVec(pose.Pos().X(), pose.Pos().Y(), pose.Pos().Z());        
        ignition::math::Vector3d goalVec(goal.Pos().X(), goal.Pos().Y(), goal.Pos().Z());
        ...
        key->Translation(ignition::math::Vector3d(curVec.X(), curVec.Y(), (curVec.Z()+10)));

        key = anim->CreateKeyFrame(distance*0.9);
        key->Translation(ignition::math::Vector3d(goalVec.X(), goalVec.Y(), (goalVec.Z()+10)));  
    ...
    public: void OnRosMsg_Pos(const abstract_drone::LocationConstPtr &_msg)
    {
    ...
    private: ros::Publisher rosPostmasterPub;    
