...
{
  private: std::string createDroneSDFstring(uint8_t droneID, float x, float y, float z)
  {
        std::stringstream ss;
        ss<<"<sdf version ='1.4'>\
          <model name ='drone'>\
  ...
  public: void createDrone (uint8_t ID, float x, float y, float z,physics::WorldPtr _parent)
  {
    sdf::SDF boxSDF;
    boxSDF.SetFromString(createDroneSDFstring(ID,x,y,z));
    // Demonstrate using a custom model name.
    sdf::ElementPtr model = boxSDF.Root()->GetElement("model");
    model->GetAttribute("name")->SetFromString("drone"+std::to_string(ID));
  ...
  public: void Load(physics::WorldPtr _parent, sdf::ElementPtr _sdf)
  {
    ...
    for(int i = 1; i < amount+1; i++)//start with 1 since the gateway already is number 0
