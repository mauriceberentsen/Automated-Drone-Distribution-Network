...
private:
                    ...
                    abstract_drone::WirelessMessage::Response &res)
  {
    ...
    if (from != Network.end() && to != Network.end()) //Already exists in the Network UpdateLocation
    {
      ROS_INFO("compare nodes %d ==> %d", req.from, req.to );
      float distance = from->second->getPosition().Distance(to->second->getPosition());
      // using pythgoras in the function Vector3 to get the distance between to nodes
...
private:
                       ...
                       abstract_drone::AreaScan::Response &res)
  {
    ...
    if (from != Network.end())
    {
      ...
      {
        if (other.first == from->first)
          continue;
        float distance = from->second->getPosition().Distance(other.second->getPosition());
        // using pythgoras in the function Vector3 to get the distance between to nodes
        if (distance < maxComDistance)
        {
...
public:
  void Load(physics::WorldPtr _parent, sdf::ElementPtr _sdf)
  {
    ...
    this->messageservice = this->rosNode->advertiseService("message", &WirelessSignalSimulator::send_message, this);
    this->service = this->rosNode->advertiseService("othersInRange", &WirelessSignalSimulator::getNodesInRange, this);
    this->rosQueueThread =
...
private:
  void addNodeToNetwork(uint8_t id, Vector3<float> &pos, std::string pubtopicname)
  {
    this->Network.insert(std::make_pair(id, new wireless::Node(pos, rosNode, pubtopicname)));
    ROS_INFO("added node [%d] on position [%f] [%f] [%f] with name %s ", id, pos.X(), pos.Y(), pos.Z(), pubtopicname.c_str());
  }
