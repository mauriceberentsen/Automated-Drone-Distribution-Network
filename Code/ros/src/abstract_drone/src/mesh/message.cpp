...
void Message::CopyToCharArray(uint8_t *value, uint16_t size, uint8_t *arr, uint16_t start)
{
...
void Message::CopyFromCharArray(uint8_t *value, uint16_t size, const uint8_t *arr, uint16_t start)
{
    for (uint16_t i = 0; i < size; i++)
    {
        value[i] = arr[i + start];
    ...
    : Message(payload[0], LOCATION)
{
    ...
    counter += sizeof(ID);
    counter += sizeof(type);
    CopyFromCharArray((uint8_t *)&latitude, sizeof(latitude), payload, counter);
    counter += sizeof(latitude);
    CopyFromCharArray((uint8_t *)&longitude, sizeof(longitude), payload, counter);
    counter += sizeof(longitude);
    CopyFromCharArray((uint8_t *)&height, sizeof(height), payload, counter);
    counter += sizeof(height);
    CopyFromCharArray((uint8_t *)&timeSincePosix, sizeof(timeSincePosix), payload, counter);
}
...
void locationMessage::toPayload(uint8_t *payload)
{
    ...
    CopyToCharArray((uint8_t *)&type, sizeof(type), payload, counter);
    counter += sizeof(type);
    CopyToCharArray((uint8_t *)&latitude, sizeof(latitude), payload, counter);
    counter += sizeof(latitude);
    CopyToCharArray((uint8_t *)&longitude, sizeof(longitude), payload, counter);
    counter += sizeof(longitude);
    CopyToCharArray((uint8_t *)&height, sizeof(height), payload, counter);
    counter += sizeof(height);
    CopyToCharArray((uint8_t *)&timeSincePosix, sizeof(timeSincePosix), payload, counter);
}
...
std::string locationMessage::toString()
{
    ...
    ss << std::setprecision(8) << std::dec << "ID[" << (int)ID << "] Type[" << (int)type << "]" << std::endl
       << "latitude[" << latitude << "] longitude[" << longitude << "] height[" << height << "] timeSincePosix[" << timeSincePosix << "]" << std::endl;
...
IntroduceMessage::IntroduceMessage(const uint8_t _ID, const uint8_t _hopsUntilsGateway)
    ...
    : Message((uint8_t)payload[0], PRESENT)
{
    ...
    counter += sizeof(ID);
    counter += sizeof(type);
    CopyFromCharArray((uint8_t *)&hopsUntilsGateway, sizeof(hopsUntilsGateway), payload, counter);
}
...
void IntroduceMessage::toPayload(uint8_t *payload)
{
    ...
    CopyToCharArray((uint8_t *)&type, sizeof(type), payload, counter);
    counter += sizeof(type);
    CopyToCharArray((uint8_t *)&hopsUntilsGateway, sizeof(hopsUntilsGateway), payload, counter);
}
...
std::string IntroduceMessage::toString()
{
    ...
    ss << "ID[" << std::to_string(ID) << "] Type[" << std::to_string(type) << "]"
       << "knowGateway[" << std::to_string(hopsUntilsGateway) << "]" << std::endl;
...
GoToLocationMessage::GoToLocationMessage(uint8_t _ID, float _latitude, float _longitude, int16_t _height)
    : Message(_ID, MOVE_TO_LOCATION), latitude(_latitude), longitude(_longitude), height(_height)
{
}
...
std::string GoToLocationMessage::toString()
{
    std::stringstream ss;
    ss << std::setprecision(8) << std::dec << "ID[" << (int)ID << "] Type[" << (int)type << "]" << std::endl
       << "latitude[" << latitude << "] longitude[" << longitude << "] height[" << height << "]" << std::endl;
